...
export class BooksService {
  ...
  /*
   * 네이버 폴백 비활성화(2026-08-08)로 더 이상 호출되지 않음. 재활성화 시 아래 전체 주석 해제.
   *
  ...
  async searchBook(isbn: searchBookDto['isbn']) {
    ...
    try {
      ...
      if (!response || response.error) {
        this.logger.warn(`도서관 빅데이터 API 조회 실패: ISBN=${isbn}`);
        return {} as any;
      }

      const detail = response.detail;
      if (!Array.isArray(detail) || detail.length === 0) {
        this.logger.warn(`도서관 빅데이터 API 결과 없음: ISBN=${isbn}`);
        return {} as any;
      }

      const foundBook = detail[0]?.book;
      if (!foundBook) {
        this.logger.warn(`도서관 빅데이터 API 북 데이터 없음: ISBN=${isbn}`);
        return {} as any;
      }
      this.logger.log(`도서 상세 조회 성공: ${foundBook.bookname}`);
      return foundBook;
    } catch (error) {
      this.logger.error(`도서 상세 조회 중 오류 발생: ISBN=${isbn}`, error);
      throw new InternalServerErrorException('도서 정보를 가져올 수 없습니다');
    }
  }
  ...
  /*
   * 네이버 폴백 비활성화(2026-08-08)로 더 이상 호출되지 않음. 재활성화 시 주석 해제.
