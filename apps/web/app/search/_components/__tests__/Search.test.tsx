...
describe("Search 컴포넌트", () => {
  ...
  it("정상적으로 렌더링되어야 한다", () => {
    render(<Search />);
    expect(screen.getByPlaceholderText("도서명을 입력하세요")).toBeInTheDocument();
  ...
  it("검색 모드(제목/ISBN)를 변경할 수 있어야 한다 (모드 변경 및 placeholder 변경 검증)", async () => {
    ...
    expect(screen.getByPlaceholderText("ISBN을 입력하세요.")).toBeInTheDocument();
  });
  ...
  it("Autocomplete에서 항목을 선택하면 해당 검색어로 이동해야 한다", async () => {
    ...
    expect(mockPush).toHaveBeenCalledWith(
      expect.stringContaining("query=%EC%B6%94%EC%B2%9C%20%EA%B2%80%EC%83%89%EC%96%B4")
    );
  });
  ...
  it("에러 상태일 때 에러 메시지를 렌더링해야 한다 (state.error 분기)", () => {
    ...
    jest.spyOn(React, "useActionState").mockReturnValue([
      { error: "검색 중 오류가 발생했습니다.", success: false, query: "", mode: "title" },
      jest.fn(),
      false
    ]);
